...
import configparser
from .paths import (
    PATH_AWS_CONFIG_FILE,
    PATH_AWS_CREDENTIAL_FILE,
)
...
def replace_section(
    ...
    with open(config_file, "rb") as f:
        ...
        for line in content.split("\n"):
            ...
            if is_in_target_section_flag \
                and (line.startswith("[") and line.endswith("]")) \
                and (line != target_section_line):
...
def overwrite_section(
    ...
    with open(config_file, "rb") as f:
        ...
        for line in content.split("\n"):
            ...
            # encounter next section after the target_section_name
            if is_in_section_flag \
                and (line.startswith("[") and line.endswith("]")) \
                and (line != section_line):
                new_lines.append("")  # add an empty line
                is_in_section_flag = False
                append_flag = True
...
def read_aws_profile_list_from_config(
    aws_config_file: str = PATH_AWS_CONFIG_FILE.abspath,
):
...
def read_aws_profile_list_from_config_with_cache(
    aws_config_file=PATH_AWS_CONFIG_FILE.abspath,
    expire=1,
):
    """
...
def set_named_profile_as_default(
    aws_profile,
    aws_config_file=PATH_AWS_CONFIG_FILE.abspath,
    aws_credentials_file=PATH_AWS_CREDENTIAL_FILE.abspath,
):  # pragma: no cover
